classdef IPM_Model
    %IPM_Model: Construct and train an Interval Predictor Model (IPM)
    ...
    methods
        ...
        function IPMobject = IPM_Model(varargin)  % method to define an object of type Scenario_Based_Reliability
            ...
            if isempty(IPMobject.options) % default optimization option if not provided
                IPMobject.options=optimoptions('fmincon','MaxIterations',1e4,'ConstraintTolerance',1e-8,...
                    'StepTolerance',1e-6,'MaxFunctionEvaluations',1e4,'Display','off','Algorithm','sqp');
            end
        end     %of constructor
        
        %% IPM Training Methods
        function [IPM] = DesingIPM_hard_constrained_noExeptions(obj,Xdn,Ydn)
            % optimize IPM bounds by enforcing a full-data enclosure between the bounds (no expetion)
            % tightness is guaranteed by minimization of an area metric
            X=obj.Normalize(Xdn,obj.Xsupport(1),obj.Xsupport(2));  % normalize in [-1 +1]
            Y=obj.Normalize(Ydn,min(Ydn),max(Ydn));                % normalize in [-1 +1]
            
            % AREA metric: approximate integral of the difference between two curves
            [~,~,Area_fun]=Def_fun_basis_objective(obj,X); % trapezoidal rule/ midpoint rule
        ...
        function [f_up,f_low,objfun]=Def_fun_basis_objective(obj,X)
            ...
            objfun= @(x) trapz(X_linspace,f_up(x)-f_low(x)); % trapezoidal integration rule
        end
        ...
        %% IPM assess validation samples and estimate reliability
    ...
    methods (Static)
      % [c,ceq]=IPM_constraints_no_exceptions(theta,Y,X,Degree_up,Degree_low,BasisType,Len);
%        [c,ceq]=IPM_Constraint_RemoveOutliers_fromTailG(theta,Y,X,Nout,Degree_up,Degree_low,BasisType,Len);
%        [c,ceq,g_val]=IPM_SoftConstraint(theta,Y,X,Degree_up,Degree_low,BasisType,Len);
%        [c,ceq]=IPM_CVaRConstraint_y_unc(x,Y,X,Degree_up,Degree_low,BasisType,Alpa_level,Std,Len);
%  
%          function   [c,ceq]=IPM_constraints_no_exceptions(theta,Y,X,Degree_up,Degree_low,BasisType,Len)
%               [c,ceq]=IPM_constraints_no_exceptions(theta,Y,X,Degree_up,Degree_low,BasisType,Len);
%          end
%         
%         function [c,ceq]=Scenario_hard_constraint_discard_tails(theta,Y,X,Degree_up,Degree_low,BasisType,Len)
%            [c,ceq]=IPM_Constraint_RemoveOutliers_fromTailG(theta,Y,X,Nout,Degree_up,Degree_low,BasisType,Len);
%         end
%         
%         function [c,ceq,g_val]=Scenario_soft_constraint_all(theta,Y,X,Degree_up,Degree_low,BasisType,Len)
%             [c,ceq,g_val]=IPM_SoftConstraint(theta,Y,X,Degree_up,Degree_low,BasisType,Len);
%         end
%         function [c,ceq]=IPM_CVaRConstraint_y_unc(x,Y,X,Degree_up,Degree_low,BasisType,Alpa_level,Std,Len)
%            [c,ceq]=IPM_CVaRConstraint_y_unc(x,Y,X,Degree_up,Degree_low,BasisType,Alpa_level,Std,Len);
%         end
